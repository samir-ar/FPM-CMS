...
class ApiRepository
{
        ...
        $n = News::find($news_id);
            ...
            $placeholder = Placeholder::where('type', 'news')->first()->image;
                return [
                    ...
                    'title' => $n->title,
                    'source' => $n->source,
                    'source_image' => $n->source_image ? Storage::disk('s3')->url(config('app.aws_bucket_project_name') . '/' . 'storage/' . 'images/news/' . $n->source_image) : Storage::disk('s3')->url(config('app.aws_bucket_project_name') . '/' . 'storage/' . 'images/placeholders/' . $placeholder),
                    'details' => $n->details,
                    'type' => $n->type,
                    'file' => Storage::disk('s3')->url(config('app.aws_bucket_project_name') . '/' . 'storage/' . 'images/news/attachments/' . $n->file),
    ...
    public function shareNews($news_id, $user)
    {
        ...
        $n->save();
            ...
            return [
                ...
                'file' => Storage::disk('s3')->url(config('app.aws_bucket_project_name') . '/' . 'storage/' . 'images/news/attachments/' . $n->file),
                'date' => strtotime($n->date),
                'link' => Storage::disk('s3')->url(config('app.aws_bucket_project_name') . '/' . 'storage/' . 'images/news/attachments/' . $n->file),
                'likes_nb' => $n->users()->count(),
                'like' => $n->users()->where('user_id', $user->id)->first() ? true : false,
                'shares' => $n->shares,
            ];
    ...
    public function getNewsInstance($n, $user, $placeholder)
    {
        ...
        return [
            ...
            'details' => $n->details,
            'type' => $n->type,
            'file' => ($n->file)?Storage::disk('s3')->url(config('app.aws_bucket_project_name') . '/' . 'storage/' . 'images/news/attachments/' . $n->file):null,
            'thumbnail' => $n->thumbnail ? Storage::disk('s3')->url(config('app.aws_bucket_project_name') . '/' . 'storage/' . 'images/news/' . $n->thumbnail) : Storage::disk('s3')->url(config('app.aws_bucket_project_name') . '/' . 'storage/' . 'images/placeholders/' . $placeholder),
            'date' => strtotime($n->date),
            'link' => ($n->file)?Storage::disk('s3')->url(config('app.aws_bucket_project_name') . '/' . 'storage/' . 'images/news/attachments/' . $n->file):null,
            'likes_nb' => $n->users()->count(),
            'like' => $n->users()->where('user_id', $user->id)->first() ? true : false,
            'shares' => $n->shares,
